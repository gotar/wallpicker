...
class FavoritesService(BaseService):
    ...
    def __init__(self, favorites_file: Optional[Path] = None) -> None:
    ...
    def _load_favorites(self) -> List[Favorite]:
        ...
        try:
            ...
            if self.favorites_file.exists():
                ...
                self._favorites = [Favorite.from_dict(data, Wallpaper) for data in favorites_data]
                self.log_debug(f"Loaded {len(self._favorites)} favorites")
            else:
                self._favorites = []
        except (json.JSONDecodeError, OSError) as e:
            self.log_error(
                f"Failed to load favorites from {self.favorites_file}: {e}", exc_info=True
            )
