...
class LocalView(Adw.BreakpointBin):
    ...
    def scroll_to_current_wallpaper(self):
        ...
        # Check if current wallpaper is in ~/Wallpapers/ (local files only)
        wallpapers_dir = self.view_model.local_service.pictures_dir
        is_local = str(current_path).startswith(str(wallpapers_dir))

        if not is_local:
            # Current wallpaper is from cache (Wallhaven), not in local list
            if self.toast_service:
                self.toast_service.show_info(
                    "Current wallpaper is from Wallhaven (not in local list)"
                )
            return
